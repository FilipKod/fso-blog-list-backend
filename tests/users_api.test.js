...
describe('if there is some users in db', () => {
  beforeEach(async () => {
    ...
    const usersToDb = await Promise.all(helper.initalUsers.map(async (user) => {
      return {
  ...
  describe('addition of new user', () => {
    ...
    test('fails with status code 400 if username is less than 2 characters', async () => {
      ...
      assert(result.body.error.includes('Username must contains at least 3 characters.'))
    })
    ...
    test('fails with status code 400 if password is less than 2 characters', async () => {
      ...
      assert(result.body.error.includes('Password must contains at least 3 characters.'))
    })
  })
